...
<!-- TODO: add a screenshot or short GIF here — a before/after of the dashboard's live feed
     with a detected plate is the single highest-leverage addition you can make to this README. -->

---

## Table of Contents

- [Why FastLPR](#why-fastlpr)
- [Architecture](#architecture)
- [Persian Plate Handling](#persian-plate-handling)
...
All of this is exposed to operators through a FastAPI backend and a single-file dashboard — see [API Reference](#api-reference) and [Dashboard](#dashboard).
...
- **Manual control** — open/close sessions.
  <img width="1359" height="644" alt="image" src="https://github.com/user-attachments/assets/8441e2ba-bac2-4dfe-8bf5-72fce16b9724" />
...
| `POST` | `/system/source` | Change the video source (`{"source": <camera index \| RTSP/HTTP URL \| file path>}`) |
| `POST` | `/system/start` | Start the capture/processing pipeline |
| `POST` | `/system/stop` | Stop the pipeline |
| `GET` | `/whitelist` | List authorized plates (`{plate, owner_name}`) |
| `POST` | `/whitelist/add` | Add a plate (`plate`, optional `owner_name`) |
| `DELETE` | `/whitelist/remove/{plate}` | Remove a plate |
| `GET` | `/logs/today?limit=&offset=` | Today's traffic history, paginated |
| `GET` | `/logs?limit=&offset=` | Full traffic history, paginated |
| `GET` | `/logs/sessions?limit=&offset=` | Vehicles currently inside (`{plate, entry_time, entry_image, confidence, owner_name}`) |
| `POST` | `/logs/sessions/add` | Manually register an entry, bypassing the camera |
| `DELETE` | `/logs/sessions/delete/{plate}` | Manually clear an active session |
| `POST` | `/logs/history/add` | Manually insert a complete history record (entry **and** exit) |
